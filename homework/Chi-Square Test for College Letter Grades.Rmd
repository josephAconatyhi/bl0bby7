...
ANSWER HERE
Even though the differences don't appear to be that high, they are meaningful as shown in the MovieRatings activity we did in class. Small differences could make up significant changes in % that we don't see as our human instinct is to group two close numbers together.
...
ANSWER HERE
Yeah the sttistic is greater than the critical value, so we reject the null hypothesis and go with the alternative one, which would be that the observed frequency is different from what we expected it to be.
...
ANSWER HERE
Expected is lower than observed at the "A" and "D" categories, and expected is higher than observed in "B". "C", and "F". Differences are as high as 20 and as low as 5 but these small differences are significant in a subtle way that a first glance wouldn't catch. Questions I'd ask about the discrepancies are how do the changes affect the data and the critical value (something that detrmines our entire outlook on the results of the data). 
