...
ANSWER HERE
Voter Turnout looks most correlated pct unemploye and pct white as well as least correlated with with income, median age, pct bachelors higher, and population. It looks like voter turnout increases as income increases and  as pct white increases. And as voter turnout decreases, pct unemployed decreases and population decreases. 
...
ANSWER HERE
Coefficients wise the income factor looks to be the highest one out of them all which is kind of true but also the scatterplot didn't really give me the impression it was the most related to voter turnout. For Residuals honestly I don't really know what to make of the numbers not gonna lie. My bad. And for  model fit; it seems a little different compared to the actual data. 
...
ANSWER HERE
Full model still looks most accurate, while the second model which only has population, pct white, and pct unemployed is second highest. Third and forth models are third and forth most accurate, third is income, median age, and population while forth only has two variables; pct unemployed and pct bachelors higher.

```{r}
# Model 2
model2 <- lm(turnout ~ population + pct_white + pct_unemployed, data = md_turnout)
summary(model2)

# Model 3
model3 <- lm(turnout ~ income + median_age + population, data = md_turnout)
summary(model3)

# Model 4
model4 <- lm(turnout ~ pct_unemployed +  pct_bachelors_higher, data = md_turnout)
summary(model4)
...
ANSWER HERE
Would use this to describe that a data set found that voter turnout increases with income level, education level, and median age. It decreases with unemployment increasing  and population rates increasing.
