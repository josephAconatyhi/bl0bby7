...
ANSWER HERE
Hard to tell what relationship is going on here, a lot of the schools have kind of scattered and jumbled FARMS % and Math scores so it sort of looks like nothing. Just a bunch of little points above and below the line of best fit with an entirely separate community of outliers at the end of the line. 
...
ANSWER HERE
The model gives some predictive information and is useful in giving an idea on how unrelated the math scores and the FARMS rate are together. Residuals too high, p value too insignificant, r squared value practically 0. All these things point to a lack of relationship that we could intuitively tell from the graph but didn't have firm evidence for outside of what we saw. 
...
ANSWER HERE
Largest differences between predicted and actual look like upwards of 34% difference between the model and actuality. Anything in the 20s and 30s just shows how little of a pattern / relationship there is between FARMS and math scores. 
...
ANSWER HERE
Conclusions... no correlation between FARMS and math tests. In a not disrespectful way I would have to say the school board member's claim is only sometimes true, and show linear regression and that graph with all the dots everywhere. I guess the story would be that there are many other factors that go into this stuff that we can't account for 100% of the time, and to assume FARMS accounts for math success.
